...
# 23 "src/adcon.c" 2
# 37 "src/adcon.c"
uint16_t adcon_leitura_min = 0xFFFF;
  ...
  if (qtd_leituras < 120) {
    for (uint8_t i = 0; i < cfg_quant_sensores_atual; i++) {
      ...
      if (t_int < menor) {
        ...
        eeprom_grava_word(0x04, t_int);
      }
      if (t_int > maior) {
        adcon_leitura_max = t_int;
        eeprom_grava_word(0x06, t_int);
      }
      eeprom_grava_word(0x10 + (qtd_leituras * 2), t_int);
      qtd_leituras++;
      adcon_print(t_int, i);
    }
    adcon_quant_amostras_gravadas++;
    eeprom_write(0x08, adcon_quant_amostras_gravadas);
  }
  else {
