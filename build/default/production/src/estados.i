...
static void est_estado_ver_todos(TBotao botao) {
  switch (botao) {
    case BTN_UP:
      ...
      break;
    case BTN_DOWN:
      if (est_ver_todos_index_amostra < adcon_quant_amostras_gravadas - 1) {
        est_ver_todos_index_amostra++;
        est_executa(EST_ESTADO_MOSTRA_TODOS);
      }
      break;
  ...
  switch (novo_estado) {
    ...
    case EST_ESTADO_MOSTRA_TODOS:
      ...
      if (adcon_quant_amostras_gravadas == 0) {
        ...
        qtd_sens = cfg_quant_sensores_atual;
        endereco = 0x10 + (est_ver_todos_index_amostra * qtd_sens * 2);

        for (uint8_t i = 0; i < qtd_sens; i++) {
            temp_int = eeprom_le_word(endereco);
    ...
    case EST_ESTADO_MOSTRA_MAX_MIN:
      ...
      lcd_clear();
      maior = eeprom_le_word(0x06);
      menor = eeprom_le_word(0x04);
      if (menor == 0xFFFF) {
            lcd_puts("Nenhum Dado");
      }
      else {
